...
const fetchNotionPage = async (PAGE_ID: string): Promise<NotionPage> => {
  const result = await fetch(`https://api.notion.com/v1/blocks/${PAGE_ID}/children`, {
    cache: 'no-store',
    headers: {
...
const getNotionPages = async () => {
  ...
  return await Promise.all(PAGE_IDs.map(id => fetchNotionPage(id)));
};
    ...
    <article>
      {page.results.map((block: Block) => {
        ...
        if (block.type === 'numbered_list_item') {
          return (
            <li key={block.id}>
              {block.numbered_list_item.rich_text.map((t: IText, index: number) => (
                <Text key={index} text={t} />
              ))}
            </li>
          );
        ...
        if (block.type === 'bulleted_list_item') {
          return (
            <li key={block.id}>
              {block.bulleted_list_item.rich_text.map((t: IText, index: number) => (
                <Text key={index} text={t} />
              ))}
            </li>
          );
...
const Page = async ({ searchParams: { token } }: { searchParams: { token: string } }) => {
  if (token !== process.env.ASSISTANT_TOKEN) return notFound();
